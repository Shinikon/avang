...
export function exportToCSV() {
  ...
  let csvContent =
    "ID;Заголовок;Описание;Статус;Приоритет;Автор;Исполнитель;Дата создания\n";
  ...
  for (const r of requests) {
    ...
    csvContent += `"${r.id}";"${cleanTitle}";"${cleanDesc}";"${
      statusLabels[r.status]
    }";"${priorityLabels[r.priority]}";"${author}";"${assignee}";"${formatDate(
      r.createdAt
    )}"\n`;
  }
